...
public class FormatterNumber: NSObject {
    ...
    public static func formatterString(value: String, minFractionLen: Int? = 0, maxFractionLen: Int? = 8, separator: String? = "") -> String {
        ...
        if value.contains(".") {
            ...
            let result = FormatterNumber.formatterDouble(value: tempValue, minFractionLen: minFractionLen, maxFractionLen: maxFractionLen, separator: separator)
            return result
        }else {
            guard let tempValue = CLongLong(value) else {
                return "0"
            }
            let result = FormatterNumber.formatterInt(value: tempValue, minIntLen: minFractionLen, maxIntLen: maxFractionLen, separator: separator)
            return result
        }
    }
