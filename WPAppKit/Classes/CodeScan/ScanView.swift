...
public class ScanView: UIView {
    ...
    public init(frame:CGRect, vstyle:ScanViewStyle) {
        ...
        case .LineMove:
            scanLineAnimation = ScanLineAnimation.instance()
            break
        case .NetGrid:
            scanNetAnimation = ScanNetAnimation.instance()
            break
        case .LineStill:
    ...
    deinit {
        if let scanLine = scanLineAnimation {
            scanLine.stopStepAnimating()
        }
        
        if let scanNet = scanNetAnimation {
            scanNet.stopStepAnimating()
        }
    }
